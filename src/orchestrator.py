    ...
    async def run(
        ...
        console.print(
            Panel(
                ...
                f"[bold cyan]Ventana de tiempo:[/bold cyan] Últimas {int(max_hours)} horas\n"
                ...
                title=f"[bold green]AUTO JOB HUNTER AI v6.0[/bold green] - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                border_style="cyan"
            )
        )
        ...
        async def _run_scraper_safe(scraper):
            ...
            try:
                res = await scraper.fetch_jobs(keywords)
                return name, res
            except Exception as e:
        ...
        for j in unprocessed:
            ...
            code_modality = detect_job_modality(j)
            ...
            if modality_pref == "Solo Remoto":
                ...
                if code_modality in ["onsite_relocate", "hybrid"]:
                    if not any(k in loc.lower() for k in ["baja california", "tijuana", "ensenada", "mexicali", "tecate", "rosarito"]):
                        discarded_onsite += 1
                        self._mark_as_processed(j)
                        continue
        ...
        async def _triage_single(job_obj):
            res = await self.triage_agent.evaluate_job(job_obj, user_english_level=english_level)
            return job_obj, res
        ...
        for job_obj, triage_res in triage_results:
            if not triage_res.is_software_role:
                ...
                console.print(f"  [bold red]✖ ROL NO SOFTWARE:[/bold red] '{job_obj['title'][:38]}' @ {job_obj['company']} [dim]({triage_res.rejection_reason})[/dim]")
                continue
            ...
            console.print(f"  [bold green]✔ APROBADA POR IA:[/bold green] '{job_obj['title'][:38]}' @ [bold white]{job_obj['company'][:25]}[/bold white] [cyan]({triage_res.workplace_location})[/cyan]")
        ...
        # ============================================================
        # FASE 6: ENTREGA MULTICANAL (TELEGRAM + EMAIL)
        # ============================================================
        ...
        console.print(f"\n[bold green][✔] Búsqueda finalizada con éxito. Total entregadas: {total_approved}[/bold green]\n")
