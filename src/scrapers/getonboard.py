            ...
            for idx, (cat_name, url) in enumerate(categories, 1):
                ...
                try:
                    ...
                    for card in job_cards[:15]:
                        ...
                        tags_el = await card.query_selector_all("span.badge, span.tag")

                        title = (await title_el.inner_text()).strip() if title_el else ""
                        company = (await company_el.inner_text()).strip() if company_el else "Startup LATAM"
                        location_val = (await location_el.inner_text()).strip() if location_el else "100% Remoto, LATAM"
                        
                        tags = []
                        for t in tags_el:
                            tags.append((await t.inner_text()).strip())
                        tags_str = ", ".join(tags) if tags else "Desarrollo Web, Software"
                        ...
                        description_text = f"Vacante de {title} en {company} ({location_val}). Tecnologías y Stack: {tags_str}. Requisitos técnicos para desarrollo de software en GetOnBoard."

                        job_item = {
                            "id": f"getonboard_{abs(hash(clean_href))}",
                            "title": title.strip(),
