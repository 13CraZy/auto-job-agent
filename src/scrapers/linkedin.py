...
class LinkedInPlaywrightScraper:
    ...
    async def fetch_jobs(self, keywords: List[str] = None) -> List[Dict[str, Any]]:
        ...
        search_configs = [
            ("Mexico (Presencial y General)", "Mexico", ""),
            ("Mexico (100% Remoto)", "Mexico", "&f_WT=2")
        ]
        ...
        async with async_playwright() as p:
            ...
            for term in search_terms:
                for label, loc, extra_param in search_configs:
                    url = f"https://www.linkedin.com/jobs/search?keywords={term.replace(' ', '%20')}&location={loc}&f_TPR=r172800{extra_param}"
                    found_in_combo = 0
                    ...
                    try:
                        ...
                        for card in job_cards[:15]:
                            try:
                                ...
                                is_remote = "remoto" in location.lower() or "remote" in location.lower() or "f_WT=2" in extra_param
                                ...
                                job_obj = {
                                    ...
                                    "description": f"Vacante de {title} en {company} ({location}). Publicada en LinkedIn.",
                                    "salary": "",
                                    "modality": "remote" if is_remote else "",
