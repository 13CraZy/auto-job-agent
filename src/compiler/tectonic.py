...
async def generate_and_compile_latex(
    ...
    if lang == "en":
        ...
        edu_ite_title, edu_ite_deg = "Ensenada Institute of Technology (ITE)", "B.S. in Computer Systems Engineering (Undergraduate Coursework Completed / Non-Graduate)"
        edu_cbtis_title, edu_cbtis_deg = "CBTis 41", "Programming Technician (Vocational Technical High School Degree)"
    else:
        ...
        edu_ite_title, edu_ite_deg = "Instituto Tecnológico de Ensenada (ITE)", "Ingeniería en Sistemas Computacionales (Estudios Superiores - Licenciatura Trunca)"
        edu_cbtis_title, edu_cbtis_deg = "CBTis 41", "Técnico en Programación (Bachillerato Tecnológico)"
    ...
    cv_tex_content = cv_tex_content.replace("{{ EDU_ITE_TITLE }}", escape_latex(edu_ite_title))
    cv_tex_content = cv_tex_content.replace("{{ EDU_ITE_DEGREE }}", escape_latex(edu_ite_deg))
    cv_tex_content = cv_tex_content.replace("{{ EDU_CBTis_TITLE }}", escape_latex(edu_cbtis_title))
    cv_tex_content = cv_tex_content.replace("{{ EDU_CBTis_DEGREE }}", escape_latex(edu_cbtis_deg))

    cv_tex_content = cv_tex_content.replace("{{ PERSONAL_NAME }}", escape_latex(personal.get("name", "Felix Iñiguez Ortiz")))
    ...
    for exp in llm_result.experiences:
        ...
        period_escaped = escape_latex(exp.period or "Enero 2025 – Presente")
        location_escaped = escape_latex(exp.location or "Ensenada, B.C., México")
...
\\vspace{{1pt}}"""
    ...
    cv_tex_filename = f"CV_Felix_Iniguez_{file_prefix}_{lang.upper()}.tex"
    ...
    cl_tex_filepath = output_dir / f"Cover_Letter_Felix_Iniguez_{file_prefix}_{lang.upper()}.tex"
    cl_pdf_filepath = output_dir / f"Cover_Letter_Felix_Iniguez_{file_prefix}_{lang.upper()}.pdf"
    ...
    if cl_template_path.exists():
        ...
        cl_tex_content = cl_tex_content.replace("{{ PERSONAL_NAME }}", escape_latex(personal.get("name", "Felix Iñiguez Ortiz")))
