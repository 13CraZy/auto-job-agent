...
class LLMClient:
    ...
    async def _adapt_single_lang(self, user_profile: dict, job_posting: dict, target_lang: str) -> LLMMatchResult:
        ...
        # 3. GOOGLE GEMINI MODEL: gemini-2.5-flash / gemini-1.5-flash
        if self.has_valid_gemini and self.gemini_client:
            for gemini_model in ["gemini-2.5-flash", "gemini-1.5-flash"]:
                try:
                    print(f"[LLMClient] Adaptando ATS con Gemini {gemini_model} [{target_lang.upper()}]...")
                    ...
                    print(f"[LLMClient] Advertencia Gemini ({gemini_model}): {e}")

        # 4. OPENROUTER MODEL: meta-llama/llama-3.1-8b-instruct
        ...
        # 5. FALLBACK DINÁMICO ATS (Engine Local Garantizado)
